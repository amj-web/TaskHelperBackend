...
class WebApplicationMessage(CreateAPIView):
    ...
    def get(self, request):
        responseData = {
            ...
            "meta-data": {
                "meta": "developed for TaskHelper Frontend",
...
class CategoryAPIView(CreateAPIView):
    ...
    # Setting the permission class for the view to
    # IsAuthenticated, meaning only authenticated users can access this view
    ...
    def get(self, request):
        ...
        # Initializing the serializer with the retrieved
        #  categories and setting the many parameter to True
    ...
    def post(self, request, format=None):
        ...
        # Returning the serializer errors and a
        # status of 400 BAD REQUEST if the serializer is not valid
...
class TodoAPIView(CreateAPIView):
    ...
    def get(self, request):
        ...
        serializer = self.serializer_class(tasks, many=True)
        # Initializing the serializer with the retrieved ToDo
        #  items and setting the many parameter to True
    ...
    def post(self, request, format=None):
        ...
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
        # Returning the serializer errors and a status of 400
        #  BAD REQUEST if the serializer is not valid


class CategorySpecificAPIView(CreateAPIView):
    # View class for handling the CRUD operations for
    #  a specific category based on its id
    ...
    def get(self, request, pk):
        ...
        if Category.objects.filter(id=pk).exists():
            ...
            # Initializing the serializer with the retrieved category
            #  and setting the many parameter to False
            ...
            # Returning a message and a status of 404 NOT FOUND
    ...
    def put(self, request, pk):
        ...
        if Category.objects.filter(id=pk).exists():
            ...
            # Returning the serializer errors and a status of 400 BAD
            # REQUEST if the serializer is not valid
        else:
            return Response(
                {"message": "Category not found!"},
                status=status.HTTP_404_NOT_FOUND
...
class TodoSpecifcAPIView(CreateAPIView):
    # View class for handling the CRUD operations
    #  for a specific ToDo item based on its id
    ...
    def get(self, request, pk):
        ...
        if ToDo.objects.filter(id=pk).exists():
            ...
            # Initializing the serializer with the retrieved
            # ToDo item and setting the many parameter to False
            ...
            # Returning a message and a status of 404 NOT FOUND if the
            #  ToDo item does not exist in the database

    def put(self, request, pk, format=None):
        # Handling PUT requests
        todo = get_object_or_404(ToDo, pk=pk)
        # Retrieving the ToDo item with the given id, or returning a
        # 404 error if it doesn't exist

        serializer = self.serializer_class(todo, data=request.data)
        # Initializing the serializer with the retrieved ToDo item
        #  and the data from the request
        ...
        if serializer.is_valid():
            ...
            if "image" in request.data:
                ...
                # Printing a message if the request data
                # does not include an image file
    ...
    def delete(self, request, pk):
        ...
        if ToDo.objects.filter(id=pk).exists():
            ...
            return Response("Todo is deleted sucessfully!",
                            status=status.HTTP_200_OK)
            # Returning a message and a status of 200 OK
        else:
            return Response(
                {"message": "Todo will this id doesn't exists!"},
                status=status.HTTP_400_BAD_REQUEST,
            )
            # Returning a message and a status of 400 BAD
            # REQUEST if the ToDo item does not exist in the database


class CategoryUserSpecifcAPIView(CreateAPIView):
    # View class for handling the CRUD operations
    #  for a specific user's categories
    ...
    def get(self, request):
        ...
        if userExists(request.user.id):
            ...
            # Initializing the serializer with the
            # retrieved categories and setting the many parameter to True
...
class TodoUserSpecifcAPIView(CreateAPIView):
    ...
    def get(self, request):
        if userExists(request.user.id):
            ...
            if len(serializer.data) < 1:
                return Response(
                    {"message": "User dont have any todo"},
                    status=status.HTTP_404_NOT_FOUND,
                )
            return Response(serializer.data, status=status.HTTP_200_OK)
        else:
...
class CategoryCurrentUserAPIView(CreateAPIView):
    ...
    def get(self, request):
        ...
        if userExists(request.user.id):
            ...
            category = Category.objects.filter(user=request.user.id)
            # Initialize serializer with the retrieved categories
            #  and set many=True since there are multiple categories
            serializer = self.serializer_class(instance=category, many=True)
            # Check if the user has any categories
            if len(serializer.data) < 1:
                # If the user doesn't have any categories,
...
class ToDoCurrentUserAPIView(CreateAPIView):
    ...
    def get(self, request):
        ...
        if userExists(request.user.id):
            ...
            # Initialize serializer with the retrieved Todos
            #  and set many=True since there are multiple Todos
            serializer = self.serializer_class(instance=Todo, many=True)
            # Check if the user has any Todos
            if len(serializer.data) < 1:
                # If the user doesn't have any Todos,
                #  return a message saying so
                return Response(
                    {"message": "User dont have any Todo"},
                    status=status.HTTP_200_OK
...
class ToDoCurrentUserDeleteAPIView(CreateAPIView):
    ...
    def delete(self, request):
        ...
        if userExists(request.user.id):
            ...
            tasks.delete()
            # Return a message indicating that all
            #  tasks were deleted successfully
            return Response(
